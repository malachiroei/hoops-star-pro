...
export function TrainingTab() {
  ...
  const handleMealUpload = () => {
    // Simulate meal upload with random description
...
function MealRecommendationCard({ meal }: MealRecommendationCardProps) {
  return (
          ...
          <div className={`p-3 rounded-xl bg-gradient-to-r ${getStatusBgColor(meal.analysis.status)} border`}>
            ...
            <div className="flex gap-2 mt-2">
              <div className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs ${
                meal.analysis.details.protein ? 'bg-green-500/20 text-green-400' : 'bg-muted/50 text-muted-foreground'
              }`}>
                <Drumstick size={12} />
                חלבון
              </div>
              <div className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs ${
                meal.analysis.details.carbs ? 'bg-green-500/20 text-green-400' : 'bg-muted/50 text-muted-foreground'
              }`}>
                <Wheat size={12} />
                פחמימות
              </div>
              <div className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs ${
                meal.analysis.details.vegetables ? 'bg-green-500/20 text-green-400' : 'bg-muted/50 text-muted-foreground'
              }`}>
                <Apple size={12} />
                ירקות
              </div>
